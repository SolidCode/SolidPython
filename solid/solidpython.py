...
def _write_code_to_file(rendered_string:str, filepath:PathStr=None, include_orig_code:bool=True) -> bool:
    try:
        ...
        out_path = Path(filepath) if filepath else calling_file.with_suffix('.scad')
        ...
        # solid.scad.
        if not filepath:
             out_path = Path().absolute() / 'solid.scad'
...
def _get_version():
    ...
    try:
        ...
        return pkg_resources.get_distribution('solidpython').version
