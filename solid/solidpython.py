...
def scad_render_to_file(scad_object: OpenSCADObject,
    ...
    return _write_code_to_file(rendered_string, filepath, out_dir, include_orig_code, suffix)
...
def _write_code_to_file(rendered_string: str, 
    ...
    try:
        ...
        if filepath:
            ...
            if not odp.exists():
                odp.mkdir()
            out_path = odp / calling_file.with_suffix(suffix).name
        else:
            out_path = calling_file.with_suffix(suffix)
